...
QtObject {
    ...
    // ============ KlipperScreen 主题颜色（使用 ThemeProvider） ============
    // 优先使用 ThemeProvider，回退到默认值

    // 背景色（根据主题动态切换）
    readonly property color bgPrimary: typeof ThemeProvider !== 'undefined' ? ThemeProvider.backgroundColor : (isDarkTheme ? "#000000" : "#F5F5F5")
    ...
    // 文字颜色（根据主题动态切换）
    readonly property color textPrimary: typeof ThemeProvider !== 'undefined' ? ThemeProvider.textColor : (isDarkTheme ? "#FFFFFF" : "#212121")
    ...
    // 强调色（使用 KlipperScreen color1 作为主强调色）
    readonly property color accent: typeof ThemeProvider !== 'undefined' ? ThemeProvider.color1 : (isDarkTheme ? "#FFEB3B" : "#F9A825")
    readonly property color accentDim: typeof ThemeProvider !== 'undefined' ? ThemeProvider.color2 : (isDarkTheme ? "#FBC02D" : "#F57F17")

    // 状态色（使用 ThemeProvider 状态颜色）
    readonly property color success: isDarkTheme ? "#00FF00" : "#4CAF50"         // 成功-绿
    readonly property color warning: typeof ThemeProvider !== 'undefined' ? ThemeProvider.warningColor : (isDarkTheme ? "#FF9800" : "#FF6F00")
    readonly property color error: typeof ThemeProvider !== 'undefined' ? ThemeProvider.errorColor : (isDarkTheme ? "#FF0000" : "#D32F2F")
